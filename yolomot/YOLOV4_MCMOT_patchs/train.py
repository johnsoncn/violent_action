...
# ----- max_id_dict read from .npy(max_id_dict.npy file) #NC UPDATE 150321
...
if __name__ == '__main__':
    ...
    # ----- Set new paths #NC UPDATE 150321
    edir= os.path.join(os.path.dirname(opt.data),opt.name)
    wdir = edir + os.sep + 'weights' + os.sep  # weights dir
    last = wdir + 'last.pt'
    best = wdir + 'best.pt'
    results_file = edir + 'results.txt'

    # ----- max_id_dict read from .npy(max_id_dict.npy file) #NC UPDATE 150321
    max_id_dict={}
    iddir=opt.data
    iddir=iddir.replace('.data','.names')
    if os.path.isfile(iddir):
        with open(iddir) as f:
        	names = f.readlines()
        for i,n in enumerate(names):
            max_id_dict[i]=i+1 #as defined in the annotations 
    print(max_id_dict)
